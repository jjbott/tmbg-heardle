...
<div class="p-3 flex-col items-evenly">
    {#if todaysGame.hasFinished}
        <div class="p-3 pb-0 flex-col items-evenly">
            <a
                href={currentHeardle.url}
                title="Listen to {currentHeardle.artist} - {currentHeardle.title} on SoundCloud"
                class="no-underline"
            >
                <div
                    class="p-2 flex items-center rounded-sm"
                    class:bg-custom-positive={todaysGame.gotCorrect}
                    class:bg-custom-mg={!todaysGame.gotCorrect}
                >
                    {#if currentHeardle.img}
                        <img
                            src={currentHeardle.img}
                            class="h-14 w-14 "
                            alt="{currentHeardle.artist} - {currentHeardle.title}"
                        />
                    {/if}
                    <div class="flex-1 mx-3 text-white">
                        {#if currentHeardle.artist}
                            <p class="">{currentHeardle.artist + ""}</p>
                            <p class="text-sm ">{currentHeardle.title + ""}</p>
                        {/if}
                    </div>
